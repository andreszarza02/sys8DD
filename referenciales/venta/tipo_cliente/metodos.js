...
const getCodigo = () => {
  $.ajax({
    ...
    $("#ticli_codigo").val(respuesta.ticli_codigo);
  });
};
    ...
    .done(function (respuesta) {
      ...
      for (objeto of respuesta) {
        tabla +=
          ...
          ")'>";
        tabla += "<td>";
        tabla += objeto.ticli_codigo;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.ticli_descripcion;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.ticli_estado;
        tabla += "</td>";
        tabla += "</tr>";
      }
...
const agregar = () => {
  ...
  getCodigo();
  $("#ticli_estado").val("ACTIVO");
  habilitarBotones(false);
  $(".activar").attr("class", "form-line activar focused");
  $("#ticli_descripcion").val("");
  $(".foco").attr("class", "form-line");
  $("#tipos_clientes").attr("style", "display: none;");
...
const abm = () => {
  $.ajax({
    ...
    url: "controlador.php",
    data: {
      ticli_codigo: $("#ticli_codigo").val(),
      ticli_descripcion: $("#ticli_descripcion").val(),
      ticli_estado: $("#ticli_estado").val(),
...
const controlVacio = () => {
  ...
  if ($("#ticli_codigo").val() == "0") {
    condicion = true;
  } else if ($("#ticli_descripcion").val() == "") {
    condicion = true;
  } else if ($("#ticli_estado").val() == "") {
    condicion = true;
  }
