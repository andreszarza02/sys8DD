...
const getCodigo = () => {
  $.ajax({
    ...
    $("#marve_codigo").val(respuesta.marve_codigo);
  });
};
    ...
    .done(function (respuesta) {
      ...
      for (objeto of respuesta) {
        tabla +=
          ...
          ")'>";
        tabla += "<td>";
        tabla += objeto.marve_codigo;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.marve_descripcion;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.marve_estado;
        tabla += "</td>";
        tabla += "</tr>";
      }
...
const agregar = () => {
  ...
  getCodigo();
  $("#marve_estado").val("ACTIVO");
  habilitarBotones(false);
  $(".activar").attr("class", "form-line activar focused");
  $("#marve_descripcion").val("");
  $(".foco").attr("class", "form-line foco");
  $("#marcas_vehiculos").attr("style", "display: none;");
...
const controlVacio = () => {
  ...
  if ($("#marve_codigo").val() == "0") {
    condicion = true;
  } else if ($("#marve_descripcion").val() == "") {
    condicion = true;
  } else if ($("#marve_estado").val() == "") {
    condicion = true;
  }
