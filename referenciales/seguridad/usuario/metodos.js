...
const getCodigo = () => {
  $.ajax({
    ...
    $("#usu_codigo").val(respuesta.usu_codigo);
  });
};
    ...
    .done(function (respuesta) {
      ...
      for (objeto of respuesta) {
        tabla +=
          ...
          ")'>";
        tabla += "<td>";
        tabla += objeto.usu_codigo;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.usu_login;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.usu_fecha;
        tabla += "</td>";
        tabla += "<td>";
...
const agregar = () => {
  ...
  $(".contra").attr("class", "form-line contra");
...
const seleccionarFila = (objetoJSON) => {
  ...
  $(".contra").attr("class", "form-line contra focused");
...
const seleccionModulo = (datos) => {
  //Enviamos los datos a su respectivo input
  Object.keys(datos).forEach((key) => {
    $("#" + key).val(datos[key]);
  });
  $("#ulModulo").html();
  $("#listaModulo").attr("style", "display: none;");
  $(".mo").attr("class", "form-line mo focused");
};
...
const getModulo = () => {
  $.ajax({
    //Solicitamos los datos a listaModulo
    method: "POST",
    url: "/sys8DD/others/complements_php/listas/listaModulo.php",
    data: {
      modu_descripcion: $("#modu_descripcion").val(),
    },
  }) //Individualizamos los datos del array y lo separamos por lista
    .done(function (lista) {
      ...
      $.each(lista, function (i, objeto) {
        if (objeto.dato1 == "NSE") {
          ...
          fila +=
            "<li class='list-group-item' onclick='seleccionModulo(" +
            JSON.stringify(objeto) +
            ")'>" +
            objeto.modu_descripcion +
            "</li>";
        }
      });
