...
$sql = "select 
            ...
            from venta_cab vc 
               ...
               join tipo_comprobante tc on tc.tipco_codigo=vc.tipco_codigo
...
try {
   ...
   $mail->addStringAttachment(
      $output,
      "8_DE_DICIEMBRE_{$cabecera['tipco_descripcion']}_Nro_{$cabecera['ven_numfactura']}.pdf"
   );

   // Contenido del correo
   $mail->isHTML(true);
   $mail->Subject = "CONFIRMACION DE NOTA DE {$cabecera['tipco_descripcion']} - Nro{$cabecera['ven_numfactura']}";
   $mail->Body = '
    <!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <div class="content">
        <h2>Factura Adjunta</h2>
        <p>Estimado(a) ' . htmlspecialchars($cabecera['cliente']) . ',</p>
        <p>Es un placer informarle que su factura número <strong>' . htmlspecialchars($cabecera['ven_numfactura']) . '</strong>, con fecha ' . htmlspecialchars($cabecera['ven_fecha']) . ', ha sido generada y se encuentra adjunta en este correo en formato PDF.</p>
        <p>Le recomendamos revisar detenidamente el documento para verificar que toda la información sea correcta.</p>
        <p>Si requeriera aclaraciones adicionales o tuviera alguna duda respecto a su factura, no dude en ponerse en contacto con nuestro equipo de atención al cliente, quienes estarán encantados de asistirle.</p>
        <p>Agradecemos su confianza y preferencia, y esperamos seguir ofreciéndole un servicio de calidad.</p>

        <div class="footer">
            <p>Este mensaje ha sido generado automáticamente. Por favor, no responda directamente a este correo.</p>
            <p>Atentamente,<br />El equipo de ' . htmlspecialchars($cabecera['emp_razonsocial']) . '</p>
        </div>
    </div>
</body>
</html>
';
   ...
   $response = array(
      "mensaje" => "LA FACTURA SE ENVIO A EL CLIENTE {$cabecera['cliente']}",
      "tipo" => "info"
   );
   ...
   $response = array(
      "mensaje" => "LA FACTURA NO SE ENVIO A EL CLIENTE {$cabecera['cliente']}",
      "tipo" => "error"
   );
