...
if (isset($_POST['operacion_cabecera'])) {
   ...
   if (strpos($error, "fecha_mayor") !== false) {
      ...
      $response = array(
         "mensaje" => "EL PEDIDO DE VENTA YA SE ENCUENTRA ASOCIADO A UN PRESUPUESTO ACTIVO",
         "tipo" => "error"
      );
   } else if (strpos($error, "asociado") !== false) {
      $response = array(
         "mensaje" => "EL PRESUPUESTO DE PRODUCCION YA SE ENCUENTRA ASOCIADO A UNA ORDEN DE PRODUCCION, EL MISMO NO SE PUEDE ANULAR",
         "tipo" => "error"
      );
   } else {
      $response = array(
   ...
   $sql = "select 
               * 
            from v_presupuesto_cab vpc 
            where vpc.pres_estado <> 'ANULADO';";
