...
if (isset($_POST['operacion_cabecera'])) {
   ...
   if (strpos($error, "repetido_produccion") !== false) {
      $response = array(
      ...
      $response = array(
         "mensaje" => "LA PRODUCCION TERMINADA YA SE ENCUENTRA ASOCIADA A UNO O VARIOS REGISTROS DE CONTROL DE CALIDAD, LA MISMA NO SE PUEDE ANULAR",
         "tipo" => "error"
      );
   } else {
      $response = array(
   ...
   $sql = "select 
               * 
            from v_produccion_terminada_cab vptc 
            where vptc.proter_estado <> 'ANULADO';";
