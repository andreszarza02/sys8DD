...
//Consulta y establece el codigo de costo produccion en cabecera
const getCodigo = () => {
  $.ajax({
    method: "POST",
    url: "controlador.php",
...
//Permite aplicar un formato de tabla a la lista de costo produccion cabecera
    ...
    .done(function (respuesta) {
      ...
      for (objeto of respuesta) {
        tabla +=
          ...
          ")'>";
        tabla += "<td>";
        tabla += objeto.copro_codigo;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.copro_fecha;
        tabla += "</td>";
        tabla += "<td>";
...
//Consulta y lista los registros de costo produccion detalle
const listarDetalle = () => {
  $.ajax({
    ...
    .done(function (respuesta) {
      //individualizamos el array de objetos y lo separamos por filas
      let tabla = "";
      let totalMontoCostoProduccion = 0;
      for (objeto of respuesta) {
      ...
      //Mostramos el monto total de costo produccion
      let lineafoot;
...
const nuevo = () => {
  ...
  $("#copro_fecha").val(getDate());
  $(".ord").attr("class", "form-line ord");
  $(".est").attr("class", "form-line est focused");
  $("#copro_estado").val("ACTIVO");
...
//Pasa parametros en el controlador para guardarlos en costo produccion cabecera
const grabar = () => {
  $.ajax({
    ...
    data: {
      copro_codigo: $("#copro_codigo").val(),
      copro_fecha: $("#copro_fecha").val(),
      copro_estado: $("#copro_estado").val(),
      orpro_codigo: $("#orpro_codigo").val(),
...
const seleccionarFila = (objetoJSON) => {
  ...
  $(".activar").attr("class", "form-line activar focused");
  $(".fecha").attr("class", "form-line fecha focused");
  $(".ord").attr("class", "form-line ord focused");
...
const seleccionOrdenProduccion = (datos) => {
  ...
  $("#ulOrdenProduccion").html();
  $("#listaOrdenProduccion").attr("style", "display: none;");
  $(".ord").attr("class", "form-line ord focused");
};

//Busca, filtra y muestra la orden produccion
const getOrdenProduccion = () => {
  $.ajax({
    //Solicitamos los datos a listaOrdenProduccionCosto
    method: "POST",
    url: "/sys8DD/others/complements_php/listasMovimientos/listaOrdenProduccionCosto.php",
    data: {
      secc_descripcion: $("#secc_descripcion").val(),
      suc_codigo: $("#suc_codigo").val(),
      emp_codigo: $("#emp_codigo").val(),
    ...
    .done(function (lista) {
      ...
      //cargamos la lista
      $("#ulOrdenProduccion").html(fila);
      //hacemos visible la lista
      $("#listaOrdenProduccion").attr(
        "style",
