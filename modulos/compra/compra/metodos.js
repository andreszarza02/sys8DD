...
// Permite aplicar un formato de tabla a la lista de compra cabecera
...
// Consulta y establece el codigo de compra en cabecera
const getCodigo = () => {
  $.ajax({
    method: "POST",
    url: "controlador.php",
    ...
    $("#comp_codigo").val(respuesta.comp_codigo);
  });
};
  ...
  $.ajax({
    ...
    .done(function (respuesta) {
      ...
      for (objeto of respuesta) {
        ...
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_fecha;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_numfactura;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_timbrado;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_timbrado_venc;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_tipofactura;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_cuota;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_montocuota;
        tabla += "</td>";
        tabla += "<td>";
        tabla += objeto.comp_interfecha;
        tabla += "</td>";
        tabla += "<td>";
...
//Consulta y lista los datos de compra detalle
const listarDetalle = () => {
  $.ajax({
    ...
    .done(function (respuesta) {
      ...
      for (objeto of respuesta) {
        ...
        if (objeto.tipit_codigo == "3") {
          totalG10 += parseFloat(objeto.compdet_precio);
        } else {
        ...
        tabla += "</td>";
        tabla += "<td>";
        tabla += new Intl.NumberFormat("us-US").format(objeto.compdet_cantidad);
        tabla += "</td>";
        tabla += "<td>";
        ...
        tabla += "</td>";
        tabla += "<td>";
        tabla += new Intl.NumberFormat("us-US").format(objeto.compdet_precio);
        tabla += "</td>";
        tabla += "<td>";
...
const nuevo = () => {
  ...
  $("#comp_estado").val("ACTIVO");
  $("#comp_fecha").val(getDate());
  $("#orcom_codigo").val("");
  $("#pro_codigo").val(0);
  $("#tipro_codigo").val(0);
  $("#pro_razonsocial").val("");
  $("#com_numfactura").val("");
  $("#comp_tipofactura").val("");
  $("#comp_cuota").val("");
  $("#comp_interfecha").val("");
  $(".orc").attr("class", "form-line orc");
...
const nuevoDetalle = () => {
  ...
  consultaAsociacion(
    "Nuevo",
    "Esta compra ya está asociado a una nota de compra; no se pueden añadir más ítems."
  );
};
...
const anular = () => {
  ...
  $("#comp_estado").val("ANULADO");
  $(".est").attr("class", "form-line est focused");
  habilitarBotones(false);
  consultaAsociacion(
    "Eliminar",
    "Esta compra ya está asociado a una nota de compra; no se pueden eliminar más ítems."
  );
};
...
const grabar = () => {
  $.ajax({
    ...
    data: {
      ...
      comp_numfactura: $("#comp_numfactura").val(),
      comp_timbrado: $("#comp_timbrado").val(),
      comp_tipofactura: $("#comp_tipofactura").val(),
      comp_cuota: $("#comp_cuota").val(),
      comp_montocuota: $("#comp_montocuota").val(),
      comp_interfecha: $("#comp_interfecha").val(),
      comp_estado: $("#comp_estado").val(),
      ...
      comp_timbrado_venc: $("#comp_timbrado_venc").val(),
...
const controlVacio = () => {
  ...
  if ($("#comp_codigo").val() == "") {
    condicion = true;
  } else if ($("#comp_fecha").val() == "") {
    condicion = true;
  } else if ($("#orcom_codigo").val() == "") {
    condicion = true;
    ...
    condicion = true;
  } else if ($("#comp_timbrado").val() == "") {
    condicion = true;
  } else if ($("#pro_timbrado_venc").val() == "") {
    condicion = true;
  } else if ($("#comp_tipofactura").val() == "") {
    condicion = true;
  } else if ($("#comp_cuota").val() == "") {
    condicion = true;
  } else if ($("#comp_montocuota").val() == "") {
    condicion = true;
  } else if ($("#comp_interfecha").val() == "") {
    condicion = true;
...
const controlVacio2 = () => {
  ...
  if ($("#dep_descripcion").val() == "") {
    ...
    condicion = true;
  } else if ($("#compdet_cantidad").val() == "") {
    condicion = true;
  } else if ($("#unime_descripcion").val() == "") {
    condicion = true;
  } else if ($("#compdet_precio").val() == "") {
    condicion = true;
  }
...
const seleccionarFila = (objetoJSON) => {
  ...
  $(".orc").attr("class", "form-line orc focused");
  $(".est").attr("class", "form-line est focused");
...
const getItem = () => {
  $.ajax({
    //Solicitamos los datos a listaItemsCompra
    method: "POST",
    url: "/sys8DD/others/complements_php/listasMovimientos/listaItemsCompra.php",
    data: {
      orcom_codigo: $("#orcom_codigo").val(),
      it_descripcion: $("#it_descripcion").val(),
    },
...
//Envia a los input de orden compra cabecera lo seleccionado en el autocompletado de orden compra cabecera
const seleccionOrdenCompra = (datos) => {
  //Enviamos los datos a su respectivo input
  Object.keys(datos).forEach((key) => {
    $("#" + key).val(datos[key]);
  });
...
//Busca, filtra y muestra las ordenes de compra cabecera
const getOrdenCompra = () => {
  $.ajax({
    //Solicitamos los datos a listaOrdenCompra
    method: "POST",
    url: "/sys8DD/others/complements_php/listasMovimientos/listaOrdenCompra.php",
    data: {
      pro_razonsocial: $("#pro_razonsocial").val(),
    },
  }) //Individualizamos los datos del array y lo separamos por lista
    .done(function (lista) {
      ...
      $.each(lista, function (i, objeto) {
        if (objeto.dato1 == "NSE") {
          ...
          fila +=
            "<li class='list-group-item' onclick='seleccionOrdenCompra(" +
            JSON.stringify(objeto) +
            ")'>" +
            objeto.orden +
            "</li>";
